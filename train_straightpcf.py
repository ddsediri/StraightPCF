...
## Dataset and loader
parser.add_argument('--cvm_ckpt', type=str, default='./pretrained_cvm/ckpt_cvm.pt')
...
parser.add_argument('--tot_its', type=int, default=2)
...
parser.add_argument('--feat_embedding_dim', type=int, default=128)
parser.add_argument('--decoder_hidden_dim', type=int, default=64)
parser.add_argument('--distance_estimation', type=eval, default=True, choices=[True, False])
...
## Training
parser.add_argument('--train_cvm_network', type=eval, default=True, choices=[True, False])
parser.add_argument('--seed', type=int, default=2020)
parser.add_argument('--logging', type=eval, default=True, choices=[True, False])
parser.add_argument('--log_root', type=str, default='./logs')
parser.add_argument('--device', type=str, default='cuda')
parser.add_argument('--max_iters', type=int, default=90000)
...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='SPCF_D%s_' % (args.dataset), postfix='_' + args.tag if args.tag is not None else '')
    ...
    shutil.copy('./train_straightpcf.py', log_dir) 
    shutil.copy('./test_straightpcf.py', log_dir) 
...
logger.info('Number of velocity modules in SC: {}'.format(cvm_ckpt['args'].num_modules))
cvm_model = CoupledVMArch(args=cvm_ckpt['args']).to(args.device)
cvm_model.load_state_dict(cvm_ckpt['state_dict'])
model = StraightPCF(cvm_model, args).to(args.device)
...
def train(it):
    ...
    optimizer.zero_grad()
    model.velocity_nets.eval()
    model.encoder.train()
    model.decoder.train()

    # Forward
    loss = model.get_supervised_loss(pcl_clean=pcl_clean,
