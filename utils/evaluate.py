...
class Evaluator(object):
    ...
    def run(self):
        ...
        for name in tqdm(pcls_name, desc='Evaluate'):
            ...
            if name_gt not in pcls_high:
                self.logger.warning('Shape `%s` not found, ignored.' % name)
                continue
            # self.logger.info('Working on shape `%s` and gt_shape `%s`.' % (name, name_gt))
            pcl_high = pcls_high[name_gt].unsqueeze(0).to(self.device)
            verts = self.meshes[name_gt]['verts'].to(self.device)
            faces = self.meshes[name_gt]['faces'].to(self.device)
        ...
        self.logger.info(results.to_string())
        self.logger.info("\nMean\n" + '\n'.join([
