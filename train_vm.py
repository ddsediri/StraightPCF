...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='VM_D%s_' % (args.dataset), postfix='_' + args.tag if args.tag is not None else '')
    ...
    shutil.copy('./train_vm.py', log_dir) 
    shutil.copy('./test_vm.py', log_dir) 
...
args.noise = '0.015'
model = VelocityModule(args).to(args.device)
...
def train(it):
    ...
    pcl_std   = batch['pcl_std'].to(args.device)
    ...
    # Forward
    loss = model.get_supervised_loss(pcl_noisy_L2=pcl_noisy_L2,
                                     pcl_noisy=pcl_noisy, 
                                     pcl_clean=pcl_clean, 
                                     pcl_std=pcl_std)
    ...
    # Logging
    # logger.info('[Train] Iter %04d | Loss DIR %.6f| Loss DIST %.6f | Loss %.6f  | Grad %.6f' % (
    #     it, loss_dir.item(), 100 * loss_dist.item(), loss.item(), orig_grad_norm,
    # ))
